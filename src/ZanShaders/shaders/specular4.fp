//Specular mapping with 4 fake lights (Zandronum).
...
vec4 Process(vec4 color)
{
	...
	vec3 lightPos = vec3(4000.0, 4000.0, 4000.0) + camerapos; //vec3(0.0, 4000.0, 0.0);
	
	vec3 norm = normalize(cross(dFdx(pixelpos.xyz),dFdy(pixelpos.xyz)));
	
	vec3 spec = specularity(lightPos, viewDir, norm, texel.a);
	
	lightPos = vec3(4000.0, 4000.0, -4000.0) + camerapos;
	spec += specularity(lightPos, viewDir, norm, texel.a);
